...
FRONTEND_BUILD = DASHBOARD_DIR / "frontend" / "dist"
RESULTS_DIR = PROJECT_ROOT / "results"
DATA_DIR = PROJECT_ROOT / "data"
CHECKPOINT_DIR = PROJECT_ROOT / "results"  # checkpoints live under results/<model>/
MLFLOW_TRACKING_URI = "file:///" + str(PROJECT_ROOT / "mlruns").replace("\\", "/")

mlflow.set_tracking_uri(MLFLOW_TRACKING_URI)
client = MlflowClient()

_static = str(FRONTEND_BUILD) if FRONTEND_BUILD.is_dir() else None
app = Flask(__name__, static_folder=_static or ".", static_url_path="")
CORS(app)
...
@app.route("/")
def index():
    if not _static or not (FRONTEND_BUILD / "index.html").is_file():
        return (
            "<p>Dashboard API running. Build the React app: "
            "<code>cd dashboard/frontend && npm ci && npm run build</code></p>"
            "<p>API docs: <code>/api/experiments</code>, <code>/api/benchmark</code></p>",
            200,
        )
...
if __name__ == "__main__":
    print("🔬 Antinoise Management Dashboard")
    print("   Backend: http://localhost:1005")
